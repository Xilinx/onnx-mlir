...
LogicalResult XFEMatMulBiasOpShapeInference(
  ...
  if (isInQuantizedDomain(op, matmulOp.getResult()))
    elementType = mlir::cast<ShapedType>(matmulOp.getResult().getType())
                      .getElementType();
...
LogicalResult XFEConvOpShapeInference(
  ...
  if (isInQuantizedDomain(op, convOp.getResult()))
    elementType = mlir::cast<ShapedType>(convOp.getResult().getType())
                      .getElementType();
...
LogicalResult XFEAveragePoolOpShapeInference(
  ...
  if (isInQuantizedDomain(op, poolOp.getResult()))
    elementType = mlir::cast<ShapedType>(poolOp.getResult().getType())
                      .getElementType();
  auto resultType = RankedTensorType::get(outputShape, elementType);
