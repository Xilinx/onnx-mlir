...
struct Convert3dMaxpoolto2dMaxpool
  ...
  LogicalResult matchAndRewrite(
    ...
    if (maxpool3d.getCeilMode() != 0 && maxpool3dInShape[3] % s[1] != 0)
      return rewriter.notifyMatchFailure(maxpool3d->getLoc(),
          "ceil_mode=1 is only supported when the height "
          "is a multiple of the height stride");
    ...
    if (heightIn != maxpool3dInShape[3]) {
      ...
      SmallVector<int64_t> cropShapeVec = {maxpool3dInShape[0], // N
      ...
      poolInput = b.onnx.slice(RankedTensorType::get(cropShapeVec, elemTy),
          ...
          /*axes=*/b.onnx.constantInt64({3}),
          /*steps=*/b.onnx.constantInt64({1}));
    }

    // Reshape: N C D H W -> N C (D*H) W
    SmallVector<int64_t> foldHeightShapeVec = {maxpool3dInShape[0], // N
        maxpool3dInShape[1],                                        // C
        maxpool3dInShape[2] * heightIn,                             // D * H
        maxpool3dInShape[4]};                                       // W
    Value foldHeightReshape =
        b.onnx.reshape(RankedTensorType::get(foldHeightShapeVec, elemTy),
            poolInput, b.onnx.constantInt64(foldHeightShapeVec));
    ...
    auto maxpoolHW = rewriter.create<ONNXMaxPoolSingleOutOp>(maxpool3d.getLoc(),
        UnrankedTensorType::get(elemTy), foldHeightReshape,
    ...
    SmallVector<int64_t> depthUnfoldShapeVec = {maxpool3dInShape[0], // N
        maxpool3dInShape[1],                                         // C
        depthDim,                                                    // D
        heightOut,                                                   // Hout
        maxpoolHWOutShape[3]};                                       // Wout
    Value depthUnfoldReshape =
        b.onnx.reshape(RankedTensorType::get(depthUnfoldShapeVec, elemTy),
    ...
    SmallVector<int64_t> depthPoolShapeVec = {maxpool3dInShape[0], // N
        maxpool3dInShape[1],                                       // C
    ...
    Value evenDepthSlice = b.onnx.slice(depthPoolTy, depthUnfoldReshape,
        /*starts=*/b.onnx.constantInt64({0}),
        /*ends=*/b.onnx.constantInt64({depthDim}),
        /*axes=*/b.onnx.constantInt64({2}),
        /*steps=*/b.onnx.constantInt64({ksizeDepth}));
    Value oddDepthSlice = b.onnx.slice(depthPoolTy, depthUnfoldReshape,
        /*starts=*/b.onnx.constantInt64({1}),
        /*ends=*/b.onnx.constantInt64({depthDim}),
        /*axes=*/b.onnx.constantInt64({2}),
        /*steps=*/b.onnx.constantInt64({ksizeDepth}));
