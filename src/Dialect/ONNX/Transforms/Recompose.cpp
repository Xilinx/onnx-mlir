...
static Value matchOptionalPrecedingAdd(
    Value v, double &betaOut, SmallVectorImpl<Operation *> &deadOps) {
...
struct RecomposeHardSigmoidFromReluPattern
  ...
  static LogicalResult finish(PatternRewriter &rewriter, ONNXReluOp reluOp,
    ...
    Location loc = mlir::FusedLoc::get(
        rewriter.getContext(), {mulOp.getLoc(), reluOp.getLoc()});
  ...
  LogicalResult matchAndRewrite(
    ...
    if (auto minOp = dyn_cast<ONNXMinOp>(afterReluUser)) {
      ...
      return finish(rewriter, reluOp, minOp, mulOp, xVal, betaPrime, alphaPrime,
          deadCandidates);
    }
...
struct RecomposeHardSigmoidFromClipMulPattern
  ...
  LogicalResult matchAndRewrite(
    ...
    double betaPrime;
    Value xVal =
        matchOptionalPrecedingAdd(clipOp.getInput(), betaPrime, deadCandidates);
