...
private:
  ...
  // INT4 test data with an ODD number of elements (5), to exercise the
  // ceil rounding of the length computation: ceil(5/2) = 3 bytes, where the
  // upper nibble of the last byte is unused padding.
  //   byte 0: -8=0x8 (LSB),  7=0x7 (MSB) → 0x78
  //   byte 1: -1=0xF (LSB),  0=0x0 (MSB) → 0x0F
  //   byte 2:  3=0x3 (LSB), pad=0x0 (MSB) → 0x03
  static constexpr int8_t kInt4OddTestValues[] = {-8, 7, -1, 0, 3};
  static constexpr size_t kInt4OddTestNumElements = 5;
  static constexpr uint8_t kInt4OddRawBytes[] = {0x78, 0x0F, 0x03};
  ...
  static bool verifyInt4Odd(const char *n, mlir::ElementsAttr a) {
    return verifyTestData<int_4, int8_t>(n, a,
        llvm::ArrayRef<int8_t>(kInt4OddTestValues, kInt4OddTestNumElements));
  }
...
public:
  ...
  // Odd element count (5): the last byte's upper nibble is unused padding, so
  // ceil(5/2) = 3 bytes are stored.  Exercises the packed sub-byte rounding.
  bool testInMemoryExternalDataInt4Odd() {
    return testInMemoryExternalData<onnx::TensorProto::INT4>(
        "testInMemoryExternalDataInt4Odd", kInt4OddRawBytes,
        sizeof(kInt4OddRawBytes), {5}, verifyInt4Odd);
  }
  ...
  // Odd element count (5): the fallback must round up ceil(5/2) = 3 bytes
  // rather than truncating to 2, which is exactly what the length-inference
  // rounding needs to get right.
  bool testFileExternalDataWithZeroLengthInt4Odd() {
    return testFileExternalDataWithZeroLength<onnx::TensorProto::INT4>(
        "testFileExternalDataWithZeroLengthInt4Odd", kInt4OddRawBytes,
        sizeof(kInt4OddRawBytes), {5}, verifyInt4Odd);
  }
  ...
  bool runAllTests() {
    ...
    allPassed = testInMemoryExternalDataInt4Odd() && allPassed;
    ...
    allPassed = testFileExternalDataWithZeroLengthInt4Odd() && allPassed;
