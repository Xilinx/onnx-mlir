...
func.func @test_gather_dynamic_indices_i16(%arg0 : tensor<3x3xf32>, %indices: tensor<1x2xi16>) -> tensor<3x1x2xf32> {
  %0 = "onnx.Gather"(%arg0, %indices) {axis = 1 : si64} : (tensor<3x3xf32>, tensor<1x2xi16>) -> tensor<3x1x2xf32>
  "func.return"(%0) : (tensor<3x1x2xf32>) -> ()
// CHECK-LABEL:   func.func @test_gather_dynamic_indices_i16(
// CHECK-SAME:                                               %[[VAL_0:.*]]: tensor<3x3xf32>,
// CHECK-SAME:                                               %[[VAL_1:.*]]: tensor<1x2xi16>) -> tensor<3x1x2xf32> {
// CHECK:           %[[VAL_2:.*]] = "tosa.const"() <{value = dense<3> : tensor<1x1xi16>}> : () -> tensor<1x1xi16>
// CHECK:           %[[VAL_3:.*]] = tosa.add %[[VAL_1]], %[[VAL_2]] : (tensor<1x2xi16>, tensor<1x1xi16>) -> tensor<1x2xi16>
// CHECK:           %[[VAL_4:.*]] = "tosa.const"() <{value = dense<0> : tensor<1x1xi16>}> : () -> tensor<1x1xi16>
// CHECK:           %[[VAL_5:.*]] = tosa.greater_equal %[[VAL_1]], %[[VAL_4]] : (tensor<1x2xi16>, tensor<1x1xi16>) -> tensor<1x2xi1>
// CHECK:           %[[VAL_6:.*]] = tosa.select %[[VAL_5]], %[[VAL_1]], %[[VAL_3]] : (tensor<1x2xi1>, tensor<1x2xi16>, tensor<1x2xi16>) -> tensor<1x2xi16>
// CHECK:           %[[VAL_7:.*]] = tosa.cast %[[VAL_6]] : (tensor<1x2xi16>) -> tensor<1x2xi32>
// CHECK:           %[[VAL_8:.*]] = "tosa.const"() <{value = dense<[1, 0]> : tensor<2xi32>}> : () -> tensor<2xi32>
// CHECK:           %[[VAL_9:.*]] = tosa.transpose %[[VAL_0]], %[[VAL_8]] : (tensor<3x3xf32>, tensor<2xi32>) -> tensor<3x3xf32>
// CHECK:           %[[VAL_10:.*]] = tosa.reshape %[[VAL_9]] {new_shape = array<i64: 1, 3, 3>} : (tensor<3x3xf32>) -> tensor<1x3x3xf32>
// CHECK:           %[[VAL_11:.*]] = tosa.reshape %[[VAL_7]] {new_shape = array<i64: 1, 2>} : (tensor<1x2xi32>) -> tensor<1x2xi32>
...
// EXCLUDE-LABEL:   func.func @test_gather_dynamic_indices_i16(
// EXCLUDE:           onnx.Gather
// EXCLUDE-NOT:       tosa.gather
}
...
// Test negative indices with i16 index type.
func.func @test_gather_axis0_neg_idx_i16(%arg0 : tensor<3x2xf32>) -> tensor<2x2x2xf32> {
  %indices = "onnx.Constant"() {value = dense<[[0, -1], [1, 2]]> : tensor<2x2xi16>} : () -> tensor<2x2xi16>
  %0 = "onnx.Gather"(%arg0, %indices) {axis = 0 : si64} : (tensor<3x2xf32>, tensor<2x2xi16>) -> tensor<2x2x2xf32>
  "func.return"(%0) : (tensor<2x2x2xf32>) -> ()
// CHECK-LABEL:   func.func @test_gather_axis0_neg_idx_i16(
// CHECK-SAME:                                             %[[VAL_0:.*]]: tensor<3x2xf32>) -> tensor<2x2x2xf32> {
// CHECK:           %[[VAL_1:.*]] = "tosa.const"() <{value = dense<{{\[\[}}0, -1], [1, 2]]> : tensor<2x2xi16>}> : () -> tensor<2x2xi16>
// CHECK:           %[[VAL_2:.*]] = "tosa.const"() <{value = dense<3> : tensor<1x1xi16>}> : () -> tensor<1x1xi16>
// CHECK:           %[[VAL_3:.*]] = tosa.add %[[VAL_1]], %[[VAL_2]] : (tensor<2x2xi16>, tensor<1x1xi16>) -> tensor<2x2xi16>
// CHECK:           %[[VAL_4:.*]] = "tosa.const"() <{value = dense<0> : tensor<1x1xi16>}> : () -> tensor<1x1xi16>
// CHECK:           %[[VAL_5:.*]] = tosa.greater_equal %[[VAL_1]], %[[VAL_4]] : (tensor<2x2xi16>, tensor<1x1xi16>) -> tensor<2x2xi1>
// CHECK:           %[[VAL_6:.*]] = tosa.select %[[VAL_5]], %[[VAL_1]], %[[VAL_3]] : (tensor<2x2xi1>, tensor<2x2xi16>, tensor<2x2xi16>) -> tensor<2x2xi16>
// CHECK:           %[[VAL_7:.*]] = tosa.cast %[[VAL_6]] : (tensor<2x2xi16>) -> tensor<2x2xi32>
// CHECK:           %[[VAL_8:.*]] = "tosa.const"() <{value = dense<[0, 1]> : tensor<2xi32>}> : () -> tensor<2xi32>
// CHECK:           %[[VAL_9:.*]] = tosa.transpose %[[VAL_0]], %[[VAL_8]] : (tensor<3x2xf32>, tensor<2xi32>) -> tensor<3x2xf32>
// CHECK:           %[[VAL_10:.*]] = tosa.reshape %[[VAL_9]] {new_shape = array<i64: 1, 3, 2>} : (tensor<3x2xf32>) -> tensor<1x3x2xf32>
// CHECK:           %[[VAL_11:.*]] = tosa.reshape %[[VAL_7]] {new_shape = array<i64: 1, 4>} : (tensor<2x2xi32>) -> tensor<1x4xi32>
...
// EXCLUDE-LABEL:   func.func @test_gather_axis0_neg_idx_i16(
// EXCLUDE:           onnx.Gather
// EXCLUDE-NOT:       tosa.gather
}
