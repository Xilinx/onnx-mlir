...
// Test 11: Native dilated depthwise at 65x65 (k3s2) should not expand to k5
// CHECK-LABEL: dwc_dilation_2_65x65
func.func @dwc_dilation_2_65x65(%arg0: tensor<1x384x65x65x!quant.uniform<i8:f32, 0.05:0>>) -> tensor<1x384x65x65x!quant.uniform<i8:f32, 0.1:0>> {
    %0 = "onnx.Constant"() {value = dense<1> : tensor<384x1x3x3xi8>} : () -> tensor<384x1x3x3x!quant.uniform<i8:f32, 0.02:0>>
    %1 = "onnx.NoValue"() {value} : () -> none
    %2 = "onnx.Conv"(%arg0, %0, %1) {auto_pad = "NOTSET", dilations = [2, 2], group = 384 : si64, kernel_shape = [3, 3], pads = [2, 2, 2, 2], strides = [1, 1]} : (tensor<1x384x65x65x!quant.uniform<i8:f32, 0.05:0>>, tensor<384x1x3x3x!quant.uniform<i8:f32, 0.02:0>>, none) -> tensor<1x384x65x65x!quant.uniform<i8:f32, 0.1:0>>
    return %2 : tensor<1x384x65x65x!quant.uniform<i8:f32, 0.1:0>>
}
// CHECK: onnx.Conv
// CHECK-SAME: dilations = [2, 2]
// CHECK-SAME: kernel_shape = [3, 3]
...
// Test 12: Native dilated depthwise at 65x65 (k3s4) should not expand to k9
// CHECK-LABEL: dwc_dilation_4_65x65
func.func @dwc_dilation_4_65x65(%arg0: tensor<1x960x65x65x!quant.uniform<i8:f32, 0.05:0>>) -> tensor<1x960x65x65x!quant.uniform<i8:f32, 0.1:0>> {
    %0 = "onnx.Constant"() {value = dense<1> : tensor<960x1x3x3xi8>} : () -> tensor<960x1x3x3x!quant.uniform<i8:f32, 0.02:0>>
    %1 = "onnx.NoValue"() {value} : () -> none
    %2 = "onnx.Conv"(%arg0, %0, %1) {auto_pad = "NOTSET", dilations = [4, 4], group = 960 : si64, kernel_shape = [3, 3], pads = [4, 4, 4, 4], strides = [1, 1]} : (tensor<1x960x65x65x!quant.uniform<i8:f32, 0.05:0>>, tensor<960x1x3x3x!quant.uniform<i8:f32, 0.02:0>>, none) -> tensor<1x960x65x65x!quant.uniform<i8:f32, 0.1:0>>
    return %2 : tensor<1x960x65x65x!quant.uniform<i8:f32, 0.1:0>>
}
// CHECK: onnx.Conv
// CHECK-SAME: dilations = [4, 4]
// CHECK-SAME: kernel_shape = [3, 3]

// -----

// Test 13: Regular conv at 65x65 with dilation=2 should still expand to k5
// CHECK-LABEL: conv_dilation_2_65x65
func.func @conv_dilation_2_65x65(%arg0: tensor<1x3x65x65x!quant.uniform<i8:f32, 0.05:0>>) -> tensor<1x8x61x61x!quant.uniform<i8:f32, 0.1:0>> {
    %0 = "onnx.Constant"() {value = dense<1> : tensor<8x3x3x3xi8>} : () -> tensor<8x3x3x3x!quant.uniform<i8:f32, 0.02:0>>
    %1 = "onnx.NoValue"() {value} : () -> none
    %2 = "onnx.Conv"(%arg0, %0, %1) {auto_pad = "NOTSET", dilations = [2, 2], group = 1 : si64, kernel_shape = [3, 3], pads = [0, 0, 0, 0], strides = [1, 1]} : (tensor<1x3x65x65x!quant.uniform<i8:f32, 0.05:0>>, tensor<8x3x3x3x!quant.uniform<i8:f32, 0.02:0>>, none) -> tensor<1x8x61x61x!quant.uniform<i8:f32, 0.1:0>>
    return %2 : tensor<1x8x61x61x!quant.uniform<i8:f32, 0.1:0>>
}
// CHECK: onnx.Conv
// CHECK-SAME: dilations = [1, 1]
// CHECK-SAME: kernel_shape = [5, 5]
