...
// CHECK:       %[[Q_IN:.+]] = "onnx.QuantizeLinear"(%arg0, {{.*}}) {axis = 1 : si64
// CHECK-SAME:  : (tensor<1x64x768xf32>, tensor<f32>, tensor<ui16>) -> tensor<1x64x768xui16>
// CHECK:       %[[GATHER:.+]] = "onnx.Gather"(%[[Q_IN]], {{.*}}) {axis = 1 : si64, onnx_node_name = "/pooler/Gather"
// CHECK-SAME:  : (tensor<1x64x768xui16>, tensor<i64>) -> tensor<1x768xui16>
// CHECK:       %[[DQ0:.+]] = "onnx.DequantizeLinear"(%[[GATHER]], {{.*}}) {axis = 1 : si64, block_size = 0 : si64, onnx_node_name = "/model/input_dequant"
// CHECK-SAME:  : (tensor<1x768xui16>, tensor<f32>, tensor<ui16>) -> tensor<1x768xf32>
// CHECK:       %Y, {{.*}}, {{.*}} = "onnx.LayerNormalization"(%[[DQ0]], {{.*}}) {axis = -1 : si64, epsilon = 1.000000e-07 : f32, onnx_node_name = "/model/layer_norm"
// CHECK-SAME:  : (tensor<1x768xf32>, tensor<768xf32>, tensor<768xf32>) -> (tensor<1x768xf32>, none, none)
// CHECK:       %[[Q0:.+]] = "onnx.QuantizeLinear"(%Y, {{.*}}) {axis = 1 : si64, block_size = 0 : si64, onnx_node_name = "/model/layer_norm_output_quant"
// CHECK-SAME:  : (tensor<1x768xf32>, tensor<f32>, tensor<ui16>) -> tensor<1x768xui16>
// CHECK:       %[[DQ1:.+]] = "onnx.DequantizeLinear"(%[[Q0]], {{.*}}) {axis = 1 : si64
// CHECK-SAME:  : (tensor<1x768xui16>, tensor<f32>, tensor<ui16>) -> tensor<1x768xf32>
// CHECK:       %[[Q1:.+]] = "onnx.QuantizeLinear"(%[[DQ1]], {{.*}}) {axis = 1 : si64
// CHECK-SAME:  : (tensor<1x768xf32>, tensor<f32>, tensor<ui16>) -> tensor<1x768xui16>
// CHECK-NOT:   "onnx.Gather"({{.*}}) : (tensor<1x64x768xf32>
// CHECK:       return %[[Q1]] : tensor<1x768xui16>
...
// CHECK:       %[[GATHER:.+]] = "onnx.Gather"(%arg0, {{.*}}) {axis = 1 : si64} : (tensor<1x64x768xf32>, tensor<i64>) -> tensor<1x768xf32>
// CHECK-NEXT:  %Y, {{.*}}, {{.*}} = "onnx.LayerNormalization"(%[[GATHER]], {{.*}}) {axis = -1 : si64, epsilon = 1.000000e-07 : f32, stash_type = 1 : si64} : (tensor<1x768xf32>, tensor<768xf32>, tensor<768xf32>) -> (tensor<1x768xf32>, none, none)
// CHECK-NOT:   "onnx.Gather"(%Y
// CHECK:       return %Y : tensor<1x768xf32>
...
// CHECK:       %Y, {{.*}}, {{.*}} = "onnx.LayerNormalization"(%arg0, {{.*}}) {axis = -1 : si64, epsilon = 1.000000e-07 : f32, stash_type = 1 : si64} : (tensor<1x64x768xf32>, tensor<768xf32>, tensor<768xf32>) -> (tensor<1x64x768xf32>, none, none)
// CHECK:       %[[OUT:.+]] = "onnx.Gather"(%Y, {{.*}}) {axis = 2 : si64} : (tensor<1x64x768xf32>, tensor<i64>) -> tensor<1x64x768xf32>
// CHECK-NOT:   "onnx.Gather"(%arg0
// CHECK:       return %[[OUT]] : tensor<1x64x768xf32>
...
// CHECK:       %Y, {{.*}}, {{.*}} = "onnx.LayerNormalization"(%arg0, {{.*}}) {axis = -1 : si64, epsilon = 1.000000e-07 : f32, stash_type = 1 : si64} : (tensor<1x64x768xf32>, tensor<768xf32>, tensor<768xf32>) -> (tensor<1x64x768xf32>, none, none)
// CHECK:       %[[OUT:.+]] = "onnx.Gather"(%Y, {{.*}}) {axis = 1 : si64} : (tensor<1x64x768xf32>, tensor<64xi64>) -> tensor<1x64x768xf32>
// CHECK-NOT:   "onnx.Gather"(%arg0
// CHECK:       return %[[OUT]] : tensor<1x64x768xf32>
