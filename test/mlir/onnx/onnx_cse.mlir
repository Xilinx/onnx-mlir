...
// COM: Identical ops that directly feed func.return must NOT be CSE'd, so each
// COM: named function output keeps its own producer SSA value (see ONNXCSE.cpp
// COM: skip-func.return-feeders rule).
func.func @test_cse_noattrs() -> (tensor<64xf32>, tensor<64xf32>, tensor<64xf32>) {
  ...
  // CHECK-COUNT-3: "onnx.DequantizeLinear"
  // CHECK: return %{{.*}}, %{{.*}}, %{{.*}}
}
